...
新建 Zotero 对象时不提供客户端生成的 `key` 或 `version`。父论文先独立创建并回读 Zotero 返回的实际 key，子笔记和附件再通过第二个独立计划绑定这些 key，从而规避 Zotero 10 本地 API 的 keyed-create 初始化故障。
...
Collection、父条目和子对象是相互独立的批准阶段。若 Collection 已存在，可跳过 Collection 创建阶段。对一次预览的批准不会自动授权下一次写入；过期摘要、版本变化、Zotero 实例变化或部分失败都不会自动重试，也不会继续生成子对象计划。
...
当前实现包含 95 项自动化测试，覆盖只读探测、授权、预览摘要、版本冲突、实例变化、去重、个人笔记保护、PDF 路径限制、证据与 PDF 获取状态、部分失败、脱敏审计、真实 Zotero keyed-create 故障模拟，以及父条目/子对象两阶段端到端流程。
